...
class Solver:
    ...
    def _search_at_depth(self, board: Board, depth: int, possible_moves: int, pv_move: int) -> Tuple[int, int]:
        ...
        while next_move:
            ...
            "Timeout check"
            if self._time_limit is not None and (time.time() - self._start_time) >= self._time_limit:
    ...
    def _report_progress_and_check_stop(self, depth: int, score: int, move: int, depth_start_time: float) -> bool:
        ...
        print(f" done. Best Move: Col {move_col} Score: {score}. Time: {depth_time:.2f}s (Total: {total_time:.2f}s)")
    ...
    def heuristic(self, board: Board) -> int:
        ...
        score -= opponent_threats_3o1e * 10
