...
def minimax(state, depth, alpha, beta, maximizingPlayer):
    ...
    if depth == 0 or is_terminal:
        if is_terminal:
            ...
            elif my_board.winning_state(state, config.PLAYER_PIECE):
                return (None, -10000000000000)
            else: # Game is over, no more valid moves
                return (None, 0)
        else: # Depth is zero
            return (None, score_position(state, config.AI_PIECE))
        
    if maximizingPlayer:
        value = -math.inf
        action = random.choice(valid_locations)
        for act in valid_locations:
            temp_state = my_board.step(state, act, config.AI_PIECE)
        ...
        action = random.choice(valid_locations)
        for act in valid_locations:
            temp_state = my_board.step(state, act, config.PLAYER_PIECE)
